...
test("ums-memory-d6q.1.4: integration contract stays JSON-first, replay-safe, and tenant-isolated", async () => {
  ...
  assert.deepEqual(Object.keys(ingestResult).sort(), [
    "accepted",
    "duplicates",
    "rejected",
    "stats",
  ...
  assert.equal(
    ...
    tenantAEvents.length + tenantBEvents.length
  );
  assert.equal(ingestResult.duplicates, 0);
  assert.equal(
    ingestResult.stats.totalEvents,
    tenantAEvents.length + tenantBEvents.length
  );
...
test("ums-memory-d6q.1.13/ums-memory-d6q.1.9: codex+claude normalization paths stay replay-safe with guardrail and SLO fields", async () => {
  const engine = await createEngine({
    seed: "integration-agent-normalization-seed",
  });
  const payload = [
    {
          ...
          messages: [
            {
              ...
              content: {
                text: "Track conflict resolution history for learner profiles",
              },
            },
            {
              ...
              message:
                "Current view should stay deterministic with replay-safe digests.",
            },
          ],
        },
      ],
...
test("ums-memory-d6q.1.8: integration replay for learner profile + identity graph stays deterministic across snapshot restore", () => {
  ...
  const profileRequest = {
    ...
    identityRefs: [
      { namespace: "email", value: "lane@example.com", isPrimary: true },
    ],
  ...
  const profileCreated = executeOperation(
  ...
  assert.deepEqual(
    afterImport.identityGraphEdges,
    beforeSnapshot.identityGraphEdges
  );

  const profileReplayAfterImport = executeOperation(
    "learner_profile_update",
    profileRequest
  );
  const edgeReplayAfterImport = executeOperation(
    "identity_graph_update",
    edgeRequest
  );
  assert.equal(profileReplayAfterImport.action, "noop");
  assert.equal(
    profileReplayAfterImport.profileDigest,
    profileCreated.profileDigest
  );
  ...
  assert.equal(
    snapshotProfile("learner-lane", "tenant-lane-a").learnerProfiles.length,
    1
  );
  assert.equal(
    snapshotProfile("learner-lane", "tenant-lane-b").learnerProfiles.length,
    1
  );
});
...
test("ums-memory-d6q.2.8: integration/replay misconception lane is idempotent, isolated, and observability-rich", () => {
  ...
  const beforeExport = snapshotProfile(
    "learner-mis-lane",
    "tenant-mis-lane-a"
  ).misconceptions;
  const exported = exportStoreSnapshot();
  ...
  importStoreSnapshot(exported);
  assert.deepEqual(
    snapshotProfile("learner-mis-lane", "tenant-mis-lane-a").misconceptions,
    beforeExport
  );

  const replayAfterImport = executeOperation(
  ...
  assert.equal(
    snapshotProfile("learner-mis-lane", "tenant-mis-lane-b").misconceptions
      .length,
    1
  );
});
...
test("ums-memory-d6q.3.8: integration/replay curriculum lane keeps deterministic ranking and explanation payloads", () => {
  ...
  const updatedA = executeOperation("curriculum_plan_update", {
    ...
    metadata: {
      explanation: {
        ...
        rationaleSteps: [
          "retain targeted practice",
          "defer until after DP refresher",
        ],
      },
    },
  });
  ...
  assert.equal(
    updatedA.planItem.metadata.explanation.summary,
  ...
  assert.equal(
    snapshotProfile("learner-cur-lane", "tenant-cur-lane-a").curriculumPlanItems
      .length,
    2
  );
  assert.equal(
    snapshotProfile("learner-cur-lane", "tenant-cur-lane-b").curriculumPlanItems
      .length,
    1
  );
});
...
test("ums-memory-d6q.4.8: integration/replay scheduler lane preserves deterministic clocks, queue metadata, and isolation", () => {
  ...
  const baseRequest = {
    ...
    metadata: {
      interactionClock: {
        tick: 1,
        lastInteractionAt: "2026-03-19T23:50:00.000Z",
      },
      sleepClock: {
        window: "nightly",
        nextConsolidationAt: "2026-03-20T03:00:00.000Z",
      },
    },
  };
  ...
  const updatedRequest = {
    ...
    metadata: {
      ...
      sleepClock: {
        window: "nightly",
        nextConsolidationAt: "2026-03-20T04:00:00.000Z",
      },
  ...
  importStoreSnapshot(exported);
  assert.deepEqual(
    snapshotProfile("learner-srs-lane", "tenant-srs-lane-a")
      .reviewScheduleEntries,
    beforeExport
  );
  ...
  assert.equal(
    snapshotProfile("learner-srs-lane", "tenant-srs-lane-a")
      .reviewScheduleEntries.length,
    1
  );
  assert.equal(
    snapshotProfile("learner-srs-lane", "tenant-srs-lane-b")
      .reviewScheduleEntries.length,
    1
  );
});
...
test("ums-memory-d6q.5.8: integration/replay policy lane enforces deterministic security and audit payload lineage", () => {
  ...
  assert.deepEqual(replay.decision.metadata.audit.checklist, [
    "incident-response",
    "rollback",
  ]);
  ...
  assert.equal(
    snapshotProfile("learner-policy-lane", "tenant-policy-lane-a")
      .policyDecisions.length,
    1
  );
  assert.equal(
    snapshotProfile("learner-policy-lane", "tenant-policy-lane-b")
      .policyDecisions.length,
    1
  );
});
...
test("ums-memory-hpl.10: integration lifecycle replay-safety stays idempotent across repeated requests", () => {
  ...
  const shadowWriteRequest = {
    ...
    ruleId,
    statement:
      "Lifecycle replay safety requires deterministic idempotency across all transitions.",
    scope: "workspace",
    confidence: 0.64,
  ...
  assert.equal(
    snapshot.replayEvaluations[0].replayEvalId,
    "reval-hpl10-idempotent"
  );
});
...
test("ums-memory-hpl.10: integration lifecycle reordered requests converge with deterministic contract errors", () => {
  ...
  const shadowWriteRequest = {
    ...
    ruleId,
    statement:
      "Reordered lifecycle requests should converge after valid replay and promotion gates.",
  ...
  const canonicalProjection = selectLifecycleProjection(
    snapshotProfile(profile, storeId)
  );

  resetStore();
  const replayBeforeShadowError = captureThrownMessage(() =>
    executeOperation("replay_eval", replayEvalRequest)
  );
  const promoteBeforeShadowError = captureThrownMessage(() =>
    executeOperation("promote", promoteRequest)
  );
  assert.equal(
  ...
  const shadowCreated = executeOperation("shadow_write", shadowWriteRequest);
  const promoteBeforeReplayError = captureThrownMessage(() =>
    executeOperation("promote", promoteRequest)
  );
  assert.equal(shadowCreated.applied[0].action, "created");
  assert.equal(
  ...
  const reorderedProjection = selectLifecycleProjection(
    snapshotProfile(profile, storeId)
  );
  assert.deepEqual(reorderedProjection, canonicalProjection);
  assert.equal(reorderedProjection.candidate?.status, "demoted");
  assert.equal(
    reorderedProjection.candidate?.promotedAt,
    "2026-03-02T11:20:00.000Z"
  );
  assert.equal(
    reorderedProjection.candidate?.demotedAt,
    "2026-03-02T11:30:00.000Z"
  );
  assert.deepEqual(reorderedProjection.candidate?.latestDemotionReasonCodes, [
    "manual_override",
    "stability_check",
  ]);
