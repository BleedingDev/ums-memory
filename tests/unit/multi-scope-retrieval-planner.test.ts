...
const loadModules = async () => {
  if (!modulesPromise) {
    ...
    transpiledDirectoryPath = mkdtempSync(
      join(tempRootDirectory, "ums-memory-retrieval-planner-")
    );
...
const makePolicyService = ({
  ...
  evaluate: (request: PolicyEvaluationRequest) => {
    ...
    return Effect.succeed({
      ...
      reasonCodes: denied
        ? ["DENIED_BY_TEST_POLICY"]
        : ["ALLOWED_BY_TEST_POLICY"],
      evaluatedAtMillis: 0,
    });
  },
});
...
const actionablePackCategories = Object.freeze([
  "do",
  "dont",
  "examples",
  "risks",
...
test("ums-memory-8as.2: retrieval planner cursor pagination is deterministic", async () => {
  ...
  try {
    ...
    const firstPage = await Effect.runPromise(
      retrievalService.retrieve(firstPageRequest)
    );
    const firstPageReplay = await Effect.runPromise(
      retrievalService.retrieve(firstPageRequest)
    );
...
test("ums-memory-8as.4: retrieval reconciles contradictory memories to the newest truth and keeps lineage metadata", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      response.hits[0]?.metadata?.chronology?.supersedesMemoryIds,
      ["memory-old-truth"]
    );
    assert.deepEqual(
      response.hits[0]?.metadata?.chronology?.contradictsMemoryIds,
      ["memory-old-truth"]
    );
    assert.deepEqual(
      response.hits[0]?.metadata?.chronology?.reconciledMemoryIds,
      ["memory-old-truth"]
    );
...
test("ums-memory-8as.4: contradiction reconciliation tie-breaking is deterministic for equal timestamps", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      firstResponse.hits[0]?.metadata?.chronology?.contradictsMemoryIds,
      ["memory-beta"]
    );
    assert.deepEqual(
      firstResponse.hits[0]?.metadata?.chronology?.reconciledMemoryIds,
      ["memory-beta"]
    );
  } finally {
    db.close();
...
test("ums-memory-8as.5: actionable pack compilation is deterministic across repeated retrieval", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      firstResponse.actionablePack,
      secondResponse.actionablePack
    );
    assert.ok(firstResponse.actionablePack);
    assert.deepEqual(firstResponse.actionablePack.do, [
      "Do: verify cursor digest before replay.",
    ]);
...
test("ums-memory-8as.5: actionable pack enforces token budget and category/source bounds", async () => {
  ...
  try {
    ...
    const storageService = storageServiceModule.makeSqliteStorageService(db);
    const categoryPrefixes = Object.freeze([
      "Do:",
      "Do not:",
      "Example:",
      "Risk:",
    ...
    for (const category of actionablePackCategories) {
      assert.ok(
        response.actionablePack[category].length <=
          actionablePackTestPerCategoryLimit
      );
    }
    assert.ok(
      response.actionablePack.sources.length <= actionablePackTestSourceLimit
    );
    assert.ok(
      estimateActionablePackTokens(response.actionablePack) <=
...
test("ums-memory-8as.6: annotation warning helper remains deterministic for stale and low-confidence signals", async () => {
  ...
  const firstWarnings =
    retrievalServiceModule.__testOnly.toActionablePackAnnotationWarnings([
      {
  ...
  try {
    ...
    upsertMemorySync(storageService, {
      ...
      payload: {
        ...
        evidencePointers: [
          {
            sourceRef: "event://explainability-rich-1",
            relationKind: "supports",
          },
          {
            sourceRef: "event://explainability-rich-2",
            relationKind: "supports",
          },
        ],
        humanWeight: 0.9,
    ...
    const retrievalResponse = await Effect.runPromise(
      retrievalService.retrieve(request)
    );
    ...
    assert.equal(
      explainabilityResponse.nextCursor,
    ...
    for (const hit of explainabilityResponse.hits) {
      ...
      for (const contribution of hit.weightedContributions) {
        assert.ok(
          contribution.signalScore >= 0 && contribution.signalScore <= 1
        );
...
test("ums-memory-8as.7: explainability reason codes include scope ranking and policy factors", async () => {
  ...
  try {
    ...
    assert.ok(
      response.hits[0]?.reasonCodes.includes("RANKING_WEIGHTED_SIGNALS")
    );
  } finally {
    db.close();
  }
...
test("ums-memory-8as.8: coding-agent release workflow prefers superseding guidance with traceable actionable output", async () => {
  ...
  try {
    ...
    for (const source of actionablePack.sources) {
      ...
      assert.ok(
        source.metadata.layer === "working" ||
          source.metadata.layer === "episodic"
      );
    }
    ...
    for (const hit of firstExplainability.hits) {
      ...
      assert.equal(
        new Set(contributionSignals).size,
  ...
  try {
    ...
    upsertMemorySync(storageService, {
      ...
      payload: {
        ...
        summary:
          "Do: quarantine flaky tests and keep retry diagnostics enabled.",
        scope: { projectId, roleId, userId },
    ...
    upsertMemorySync(storageService, {
      ...
      payload: {
        ...
        summary:
          "Do not merge flaky fixes before reproducing the failing seed.",
        scope: { projectId },
        updatedAtMillis: 11 * dayMillis,
      },
    });
    upsertMemorySync(storageService, {
      ...
      payload: {
        ...
        summary:
          "Example: run tests with --runInBand and seed 42 for deterministic repro.",
        scope: { roleId },
    ...
    assert.ok(
      correctedExplainabilityHit?.reasonCodes.includes("SCOPE_LEVEL_USER")
    );
