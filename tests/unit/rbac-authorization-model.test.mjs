...
const loadAuthorizationModules = async () => {
  if (!modulesPromise) {
    ...
    const errorsModuleUrl = pathToFileURL(join(transpiledDirectoryPath, "errors.js")).href;
...
const evaluate = (service, request) => Effect.runPromise(service.evaluate(request));
...
test("ums-memory-a9v.1: admin is allowed to run privileged policy override action", async () => {
  ...
  const service = authorizationServiceModule.makeDeterministicAuthorizationService();
