...
import {
  ...
  main,
  parseFeedbackEvents,
} from "../../scripts/generate-pilot-kpi-dashboard.ts";
...
test("pilot KPI dashboard includes required schema sections", () => {
  const dashboard = generatePilotKpiDashboard(
    [SUMMARY_A],
    parseFeedbackEvents(FEEDBACK_A)
  );
  ...
  assert.equal(
    dashboard.telemetryWindow.feedbackStart,
    "2026-03-01T10:05:00.000Z"
  );
  assert.equal(
    dashboard.telemetryWindow.feedbackEnd,
    "2026-03-01T10:25:00.000Z"
  );
});
