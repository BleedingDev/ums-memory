...
import {
  ...
  main,
} from "../../scripts/evaluate-production-slos.ts";
...
test("production SLO evaluation is deterministic across equivalent dashboard ordering", () => {
  const resultA = evaluateProductionSlos([DASHBOARD_PASS_A, DASHBOARD_PASS_B], {
    evaluatedAt: EVALUATED_AT,
    dashboardSources: ["z-day-2.json", "a-day-1.json"],
  });
  const resultB = evaluateProductionSlos([DASHBOARD_PASS_B, DASHBOARD_PASS_A], {
    evaluatedAt: EVALUATED_AT,
...
test("production SLO evaluation records tuning guardrail metadata deterministically", () => {
  ...
  assert.equal(
    result.inputs.tuningSchemaVersion,
    "pilot_ranking_decay_tuning.v1"
  );
  assert.equal(result.inputs.tuningSource, "fixtures/tuning.json");

  assert.equal(
    result.thresholds.successRate.source,
    "baseline_and_tuning_guardrails"
  );
  assert.equal(
    result.thresholds.failureRate.source,
    "baseline_and_tuning_guardrails"
  );
  assert.equal(
    result.thresholds.p95LatencyMs.source,
    "baseline_and_tuning_guardrails"
  );
  assert.equal(
    result.thresholds.incidentsPer1kRequests.source,
    "baseline_and_tuning_guardrails"
  );
  assert.equal(
    result.thresholds.recallFailureRate.source,
    "baseline_and_tuning_guardrails"
  );
  assert.equal(
    result.thresholds.anomaliesPer1kRequests.source,
    "baseline_and_tuning_guardrails"
  );
  assert.equal(
    result.thresholds.policyReviewRate.source,
    "baseline_and_tuning_guardrails"
  );

  assert.equal(
    result.thresholds.successRate.tuningDerivedFromMaxFailureRate,
    0.006
  );
...
test("production SLO evaluation CLI writes compact output file", async () => {
  const fixtureRoot = await mkdtemp(
    resolve(tmpdir(), "production-slo-evaluation-cli-")
  );
  ...
  const outputPath = resolve(
    fixtureRoot,
    "nested",
    "production-slo-evaluation.json"
  );

  try {
    await writeFile(
      dashboardPath,
      `${JSON.stringify(DASHBOARD_PASS_A)}\n`,
      "utf8"
    );
...
test("production SLO evaluation CLI returns exit code 2 on failed objectives", async () => {
  const fixtureRoot = await mkdtemp(
    resolve(tmpdir(), "production-slo-evaluation-cli-fail-")
  );
