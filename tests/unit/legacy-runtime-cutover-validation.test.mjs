...
} from "../../scripts/validate-legacy-runtime-cutover.mjs";
...
test("legacy runtime cutover validation passes for allowed mjs importer patterns", async () => {
  ...
  try {
    await writeProjectFile(projectRoot, SHIM_PATH, "export function run() { return true; }\n");
    await writeProjectFile(
      projectRoot,
      "scripts/tool.mjs",
      "import { run } from \"../apps/api/src/core.mjs\";\nvoid run;\n",
    );
    const inventoryPath = await writeInventory(projectRoot, [makeInventoryEntry(SHIM_PATH)]);
    const result = await validateLegacyRuntimeCutover({ projectRoot, inventoryPath });
...
test("legacy runtime cutover validation rejects strict TypeScript imports of legacy shims", async () => {
  const projectRoot = await mkdtemp(resolve(tmpdir(), "legacy-cutover-ts-violation-"));
...
test("legacy runtime cutover validation rejects .tsx strict TypeScript imports of legacy shims", async () => {
  const projectRoot = await mkdtemp(resolve(tmpdir(), "legacy-cutover-tsx-violation-"));
...
test("legacy runtime cutover validation rejects unexpected importer locations", async () => {
  const projectRoot = await mkdtemp(resolve(tmpdir(), "legacy-cutover-unexpected-importer-"));
...
test("legacy runtime cutover validation ignores commented import references", async () => {
  const projectRoot = await mkdtemp(resolve(tmpdir(), "legacy-cutover-comments-"));
...
test("legacy runtime cutover validation captures multiple real imports in one file while ignoring comments", async () => {
  const projectRoot = await mkdtemp(resolve(tmpdir(), "legacy-cutover-multi-import-"));

  try {
    const secondShimPath = "apps/cli/src/index.mjs";
    await writeProjectFile(projectRoot, SHIM_PATH, "export function run() { return true; }\n");
    await writeProjectFile(projectRoot, secondShimPath, "export function runCli() { return true; }\n");
    await writeProjectFile(
      projectRoot,
      "scripts/multi.mjs",
      [
