...
const loadContractsModule = async () => {
  if (!contractsModulePromise) {
    ...
    transpiledDirectoryPath = mkdtempSync(join(tempRootDirectory, "ums-memory-retrieval-pack-"));
...
const decodeActionablePack = (contractsModule, input) =>
  Schema.decodeUnknownSync(contractsModule.ActionableRetrievalPackSchema)(input);
...
const makeValidPackPayload = () => ({
  ...
  examples: ["When replaying events, derive idempotencyKey from deterministic source digest."],
  ...
  warnings: ["Evidence pointer may be stale and should be revalidated before promotion."],
});
