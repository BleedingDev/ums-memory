...
import {
  ...
  main,
} from "../../scripts/generate-ranking-decay-tuning.mjs";
...
test("pilot ranking/decay tuning rejects malformed summary and dashboard inputs", async () => {
  ...
  const fixtureRoot = await mkdtemp(resolve(tmpdir(), "pilot-ranking-tuning-malformed-"));
  ...
  try {
    ...
    await writeFile(dashboardPath, '{"schemaVersion":"pilot_kpi_dashboard.v1",', "utf8");
...
test("pilot ranking/decay tuning main writes compact output with feedback NDJSON", async () => {
  const fixtureRoot = await mkdtemp(resolve(tmpdir(), "pilot-ranking-tuning-cli-"));
  const summaryPath = resolve(fixtureRoot, "summary.json");
  const dashboardPath = resolve(fixtureRoot, "dashboard.json");
