...
} from "../../scripts/validate-vector-retrieval-evaluation.mjs";
...
async function writeRunbook(projectRoot, markdown) {
  const runbookPath = resolve(projectRoot, "docs/runbooks/vector-retrieval-extension-evaluation.md");
...
test("vector retrieval runbook validation fails on missing required content rule", async () => {
  ...
  try {
    const runbookPath = await writeRunbook(
      projectRoot,
      makeRunbookContent({ dropContentLine: "semantic recall improvement >= 8%" }),
    );
    const result = await validateVectorRetrievalEvaluation({ runbookPath });
