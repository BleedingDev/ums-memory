...
} from "../../scripts/validate-cross-repo-federation-model.mjs";
...
function makeRunbookContent({ dropHeading = null, dropPhrase = null, dropContentLine = null } = {}) {
  const headingLines = REQUIRED_HEADINGS.filter((heading) => heading !== dropHeading);
  const phraseLines = REQUIRED_PHRASES.filter((phrase) => phrase !== dropPhrase);
  const contentLines = REQUIRED_CONTENT_LINES.filter((line) => line !== dropContentLine);
...
async function writeRunbook(projectRoot, markdown) {
  const runbookPath = resolve(projectRoot, "docs/runbooks/cross-repo-memory-federation-model.md");
...
test("cross-repo federation runbook validation passes when required headings and phrases exist", async () => {
  const projectRoot = await mkdtemp(resolve(tmpdir(), "federation-model-pass-"));
...
test("cross-repo federation runbook validation fails on missing required heading", async () => {
  const projectRoot = await mkdtemp(resolve(tmpdir(), "federation-model-heading-"));
...
test("cross-repo federation runbook validation fails on missing required phrase", async () => {
  const projectRoot = await mkdtemp(resolve(tmpdir(), "federation-model-phrase-"));

  try {
    const missingPhrase = "tenant isolation";
    const runbookPath = await writeRunbook(projectRoot, makeRunbookContent({ dropPhrase: missingPhrase }));
    const result = await validateCrossRepoFederationModel({ runbookPath });
...
test("cross-repo federation runbook validation fails on missing content guardrail", async () => {
  const projectRoot = await mkdtemp(resolve(tmpdir(), "federation-model-content-"));

  try {
    const runbookPath = await writeRunbook(
      projectRoot,
      makeRunbookContent({ dropContentLine: "cross-tenant leak incidents == 0" }),
    );
    const result = await validateCrossRepoFederationModel({ runbookPath });
...
test("cross-repo federation runbook validation CLI main returns failure on unknown arguments", async () => {
