...
} from "../../scripts/generate-pilot-rollout-report.ts";
...
test("pilot rollout report is deterministic for equivalent telemetry payloads", () => {
  ...
  const fromArray = generatePilotRolloutReport(
    parseTelemetryEvents(jsonArrayInput)
  );
  const fromNdjson = generatePilotRolloutReport(
    parseTelemetryEvents(ndjsonInput)
  );
...
test("pilot rollout report main allow-invalid skips malformed NDJSON lines", async () => {
  const fixtureRoot = await mkdtemp(
    resolve(tmpdir(), "pilot-rollout-report-allow-invalid-")
  );
  const inputPath = resolve(fixtureRoot, "telemetry.ndjson");
...
test("pilot rollout report rejects malformed JSON arrays even in allow-invalid mode", async () => {
  const fixtureRoot = await mkdtemp(
    resolve(tmpdir(), "pilot-rollout-report-bad-array-")
  );
...
test("pilot rollout report preserves parsed invalid counts in default mode", () => {
  const parsed = parseTelemetryEvents([FIXTURE_EVENTS[0], "invalid-entry"], {
    allowInvalid: true,
  });
