...
function isPolicyReviewOrDeny(policyDecision) {
  ...
  return normalized === "review" || normalized === "deny" || normalized === "denied";
}

function assertApproxEqual(actual, expected, label, tolerance = 1e-6) {
  if (Math.abs(actual - expected) > tolerance) {
    throw new Error(`Inconsistent derived field: ${label}. Expected ${expected}, received ${actual}.`);
  }
}
...
function readDashboard(dashboard, sourceLabel) {
  ...
  const successCount = readNonNegativeInteger(
    readRequiredField(adoption, "successCount", `${sourceLabel}.adoption.successCount`),
    `${sourceLabel}.adoption.successCount`,
  );
  const failureCount = readNonNegativeInteger(
    readRequiredField(adoption, "failureCount", `${sourceLabel}.adoption.failureCount`),
  ...
  const successRate = readRate(
    readRequiredField(adoption, "successRate", `${sourceLabel}.adoption.successRate`),
  ...
  const expectedSuccessRate = requestVolume > 0 ? round(successCount / requestVolume) : 0;
  const expectedFailureRate = requestVolume > 0 ? round(failureCount / requestVolume) : 0;
  assertApproxEqual(successRate, expectedSuccessRate, `${sourceLabel}.adoption.successRate`);
  assertApproxEqual(failureRate, expectedFailureRate, `${sourceLabel}.adoption.failureRate`);
  ...
  const anomalyHistogram = readHistogram(
    readRequiredField(quality, "anomalyHistogram", `${sourceLabel}.quality.anomalyHistogram`),
    `${sourceLabel}.quality.anomalyHistogram`,
  );
  const policyDecisionHistogram = readHistogram(
    readRequiredField(quality, "policyDecisionHistogram", `${sourceLabel}.quality.policyDecisionHistogram`),
  ...
  const expectedIncidentsPer1k = requestVolume > 0 ? round((incidentCount * 1000) / requestVolume) : 0;
  assertApproxEqual(
    incidentsPer1kRequests,
  ...
  const recallRequestVolume = readNonNegativeInteger(
    readRequiredField(recallQuality, "recallRequestVolume", `${sourceLabel}.recallQuality.recallRequestVolume`),
    `${sourceLabel}.recallQuality.recallRequestVolume`,
  );
  const recallFailureCount = readNonNegativeInteger(
    readRequiredField(recallQuality, "recallFailureCount", `${sourceLabel}.recallQuality.recallFailureCount`),
  ...
  const expectedRecallFailureRate = recallRequestVolume > 0 ? round(recallFailureCount / recallRequestVolume) : 0;
  assertApproxEqual(
    recallFailureRate,
...
function readTuningRecommendation(recommendation, sourceLabel) {
  ...
  const schemaVersion = readRequiredField(recommendation, "schemaVersion", `${sourceLabel}.schemaVersion`);
  ...
  const guardrails = readRequiredField(recommendation, "guardrails", `${sourceLabel}.guardrails`);
  if (!isRecord(guardrails)) {
    throw new Error(`Missing required object field: ${sourceLabel}.guardrails.`);
  }

  return {
    maxFailureRate: readRate(
      readRequiredField(guardrails, "maxFailureRate", `${sourceLabel}.guardrails.maxFailureRate`),
      `${sourceLabel}.guardrails.maxFailureRate`,
    ),
    maxRecallFailureRate: readRate(
      readRequiredField(guardrails, "maxRecallFailureRate", `${sourceLabel}.guardrails.maxRecallFailureRate`),
      `${sourceLabel}.guardrails.maxRecallFailureRate`,
    ),
    maxP95LatencyMs: readNonNegativeNumber(
      readRequiredField(guardrails, "maxP95LatencyMs", `${sourceLabel}.guardrails.maxP95LatencyMs`),
      `${sourceLabel}.guardrails.maxP95LatencyMs`,
    ),
    maxIncidentsPer1kRequests: readNonNegativeNumber(
      readRequiredField(guardrails, "maxIncidentsPer1kRequests", `${sourceLabel}.guardrails.maxIncidentsPer1kRequests`),
      `${sourceLabel}.guardrails.maxIncidentsPer1kRequests`,
    ),
    maxAnomaliesPer1kRequests: readNonNegativeNumber(
      readRequiredField(guardrails, "maxAnomaliesPer1kRequests", `${sourceLabel}.guardrails.maxAnomaliesPer1kRequests`),
      `${sourceLabel}.guardrails.maxAnomaliesPer1kRequests`,
    ),
    maxPolicyReviewRate: readRate(
      readRequiredField(guardrails, "maxPolicyReviewRate", `${sourceLabel}.guardrails.maxPolicyReviewRate`),
      `${sourceLabel}.guardrails.maxPolicyReviewRate`,
    ),
    abortOnRollbackSignal: readBoolean(
      readRequiredField(guardrails, "abortOnRollbackSignal", `${sourceLabel}.guardrails.abortOnRollbackSignal`),
...
function buildThresholds(tuningRecommendation) {
  ...
  const minSuccessRate = tuningRecommendation
    ? Math.max(BASE_THRESHOLDS.successRate, round(1 - tuningRecommendation.maxFailureRate))
    : BASE_THRESHOLDS.successRate;
  const maxP95LatencyMs = tuningRecommendation
    ? Math.min(BASE_THRESHOLDS.p95LatencyMs, tuningRecommendation.maxP95LatencyMs)
    : BASE_THRESHOLDS.p95LatencyMs;
  const maxIncidentsPer1kRequests = tuningRecommendation
    ? Math.min(BASE_THRESHOLDS.incidentsPer1kRequests, tuningRecommendation.maxIncidentsPer1kRequests)
    : BASE_THRESHOLDS.incidentsPer1kRequests;
  const maxRecallFailureRate = tuningRecommendation
    ? Math.min(BASE_THRESHOLDS.recallFailureRate, tuningRecommendation.maxRecallFailureRate)
    : BASE_THRESHOLDS.recallFailureRate;
  const maxAnomaliesPer1kRequests = tuningRecommendation
    ? Math.min(BASE_THRESHOLDS.anomaliesPer1kRequests, tuningRecommendation.maxAnomaliesPer1kRequests)
    : BASE_THRESHOLDS.anomaliesPer1kRequests;
  const maxPolicyReviewRate = tuningRecommendation
    ? Math.min(BASE_THRESHOLDS.policyReviewRate, tuningRecommendation.maxPolicyReviewRate)
    : BASE_THRESHOLDS.policyReviewRate;

  const source = tuningRecommendation ? "baseline_and_tuning_guardrails" : "baseline";
  ...
  return {
    successRate: {
      ...
      tuningDerivedFromMaxFailureRate: tuningRecommendation ? tuningRecommendation.maxFailureRate : null,
      source,
    },
    failureRate: {
      operator: "<=",
    ...
    p95LatencyMs: {
      ...
      tuning: tuningRecommendation ? tuningRecommendation.maxP95LatencyMs : null,
      source,
    },
    incidentsPer1kRequests: {
      operator: "<=",
      ...
      tuning: tuningRecommendation ? tuningRecommendation.maxIncidentsPer1kRequests : null,
      source,
    },
    recallFailureRate: {
      operator: "<=",
      ...
      tuning: tuningRecommendation ? tuningRecommendation.maxRecallFailureRate : null,
      source,
    },
    anomaliesPer1kRequests: {
      operator: "<=",
      ...
      tuning: tuningRecommendation ? tuningRecommendation.maxAnomaliesPer1kRequests : null,
      source,
    },
    policyReviewRate: {
      operator: "<=",
      ...
      tuning: tuningRecommendation ? tuningRecommendation.maxPolicyReviewRate : null,
      source,
    },
  };
}
...
function aggregateMeasurements(dashboards) {
  ...
  for (const dashboard of dashboards) {
    ...
    const candidateLatency = dashboard.p95LatencyMs ?? dashboard.p95LatencyEstimateMs;
    if (candidateLatency !== null) {
      latencyContributors += 1;
      p95LatencyMs = p95LatencyMs === null ? candidateLatency : Math.max(p95LatencyMs, candidateLatency);
    }
  }
  ...
  return {
    ...
    incidentCount,
    incidentsPer1kRequests: roundPerThousandOrNull(incidentCount, requestVolume),
    recallRequestVolume,
    recallFailureCount,
    ...
    return [
      {
        priority: 1,
        objective: "all",
        action: "SLO objectives are met. Continue daily evaluation and monitor trend drift.",
      },
    ];
  }
...
export function evaluateProductionSlos(
  ...
  if (dashboardSources.length > 0 && dashboardSources.length !== dashboards.length) {
    throw new Error("dashboardSources length must match dashboards length when provided.");
  }
  ...
  const sortedSources = dashboardSources.length > 0 ? [...dashboardSources].sort(compareStrings) : [];
...
function parseArgs(argv) {
  ...
  if (!parsed.help && parsed.dashboard.length === 0) {
    throw new Error("Missing required --dashboard <path> argument (repeat for multiple dashboard files).");
  }
...
function printUsage() {
  process.stdout.write(
    [
      "Usage:",
      "  node scripts/evaluate-production-slos.mjs --dashboard <dashboard.json> [--dashboard <dashboard.json> ...]",
      ...
      "",
      "Example:",
      "  node scripts/evaluate-production-slos.mjs --dashboard docs/reports/pilot-rollout/pilot-a-kpi-dashboard.json \\",
...
export async function main(argv = process.argv.slice(2)) {
  ...
  if (parsedArgs.tuning) {
    ...
    tuningRecommendation = parseJsonObject(rawTuning, parsedArgs.tuning, "tuning recommendation");
  }
