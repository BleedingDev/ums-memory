...
const VALID_ENTRY_KINDS = new Set(["runtime_entrypoint", "legacy_shared_contract"]);
...
function parseInventoryEntry(entry, index) {
  ...
  if (!VALID_ENTRY_KINDS.has(kindValue)) {
    throw new Error(
      `entries[${index}].kind must be one of: ${Array.from(VALID_ENTRY_KINDS).sort(compareStrings).join(", ")}.`,
    );
  }

  const followUpBeadId = typeof entry.followUpBeadId === "string" ? entry.followUpBeadId.trim() : "";
...
async function loadInventory(inventoryPath) {
  ...
  const entries = rawEntries.map((entry, index) => parseInventoryEntry(entry, index));
  return {
...
function compareExpectedAndActual(expectedPaths, actualPaths) {
  ...
  return {
    missingFromInventory: actualPaths.filter((entryPath) => !expected.has(entryPath)),
...
export async function validateLegacyRuntimeShims({
  ...
  const expectedPaths = inventory.entries.map((entry) => entry.path).sort(compareStrings);
...
function printUsage() {
  process.stdout.write(
    [
      "Usage:",
      "  node scripts/validate-legacy-runtime-shims.mjs [--project-root <path>] [--inventory <path>] [--json]",
      "",
      "Options:",
...
function renderFailureSummary(result) {
  ...
  if (result.duplicateInventoryEntries.length > 0) {
    lines.push(`- Duplicate inventory entries: ${result.duplicateInventoryEntries.join(", ")}`);
  }
  if (result.invalidFollowUpBeadIds.length > 0) {
    lines.push(`- Missing or invalid followUpBeadId: ${result.invalidFollowUpBeadIds.join(", ")}`);
  }
