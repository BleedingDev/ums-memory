...
import { parseFeedbackEvents } from "./generate-pilot-kpi-dashboard.mjs";
...
function readHistogram(value, label) {
  ...
  for (const [key, rawCount] of Object.entries(value)) {
    histogram.set(String(key), readNonNegativeInteger(rawCount, `${label}.${key}`));
  }
  return histogram;
}
...
function readPerOperation(value, label) {
  ...
  for (const [operation, stats] of Object.entries(value)) {
    ...
    if (!isRecord(stats)) {
      throw new Error(`Per-operation entry must be an object (${operationLabel}).`);
    }
    const requestVolume = readNonNegativeInteger(
      readRequiredField(stats, "requestVolume", `${operationLabel}.requestVolume`),
      `${operationLabel}.requestVolume`,
    );
    const successCount = readNonNegativeInteger(
      readRequiredField(stats, "successCount", `${operationLabel}.successCount`),
      `${operationLabel}.successCount`,
    );
    const failureCount = readNonNegativeInteger(
      readRequiredField(stats, "failureCount", `${operationLabel}.failureCount`),
    ...
    readRate(readRequiredField(stats, "successRate", `${operationLabel}.successRate`), `${operationLabel}.successRate`);
    readRate(readRequiredField(stats, "failureRate", `${operationLabel}.failureRate`), `${operationLabel}.failureRate`);
    readLatency(readRequiredField(stats, "p95LatencyMs", `${operationLabel}.p95LatencyMs`), `${operationLabel}.p95LatencyMs`);
    readNonNegativeInteger(
      readRequiredField(stats, "latencySampleSize", `${operationLabel}.latencySampleSize`),
      `${operationLabel}.latencySampleSize`,
    );
    const failureCodeHistogram = readHistogram(
      readRequiredField(stats, "failureCodeHistogram", `${operationLabel}.failureCodeHistogram`),
...
function readPilotSummary(summary, sourceLabel) {
  ...
  const schemaVersion = readRequiredField(summary, "schemaVersion", `${sourceLabel}.schemaVersion`);
  ...
  readRate(readRequiredField(summary, "successRate", `${sourceLabel}.successRate`), `${sourceLabel}.successRate`);
  readRate(readRequiredField(summary, "failureRate", `${sourceLabel}.failureRate`), `${sourceLabel}.failureRate`);
  ...
  const latencySampleSize = readNonNegativeInteger(
    readRequiredField(summary, "latencySampleSize", `${sourceLabel}.latencySampleSize`),
    `${sourceLabel}.latencySampleSize`,
  );

  const anomalyHistogram = readHistogram(
    readRequiredField(summary, "anomalyHistogram", `${sourceLabel}.anomalyHistogram`),
    `${sourceLabel}.anomalyHistogram`,
  );
  const policyDecisionHistogram = readHistogram(
    readRequiredField(summary, "policyDecisionHistogram", `${sourceLabel}.policyDecisionHistogram`),
  ...
  const telemetryWindowRaw = readRequiredField(summary, "telemetryWindow", `${sourceLabel}.telemetryWindow`);
  if (!isRecord(telemetryWindowRaw)) {
    throw new Error(`Missing required object field: ${sourceLabel}.telemetryWindow.`);
  }
  const telemetryStart = readTimestampOrNull(
    readRequiredField(telemetryWindowRaw, "start", `${sourceLabel}.telemetryWindow.start`),
    `${sourceLabel}.telemetryWindow.start`,
  );
  const telemetryEnd = readTimestampOrNull(
    readRequiredField(telemetryWindowRaw, "end", `${sourceLabel}.telemetryWindow.end`),
    `${sourceLabel}.telemetryWindow.end`,
  );
  if ((telemetryStart === null) !== (telemetryEnd === null)) {
    throw new Error(`Telemetry window must contain both start and end timestamps or both null (${sourceLabel}).`);
  }
  if (telemetryStart && telemetryEnd && compareStrings(telemetryStart, telemetryEnd) > 0) {
    throw new Error(`Telemetry window start must be <= end (${sourceLabel}).`);
  }
  if (requestVolume > 0 && (!telemetryStart || !telemetryEnd)) {
    throw new Error(`Telemetry window cannot be null when requestVolume > 0 (${sourceLabel}).`);
  }
  if (requestVolume === 0 && (telemetryStart || telemetryEnd)) {
    throw new Error(`Telemetry window must be null when requestVolume is 0 (${sourceLabel}).`);
  }
...
function readDashboard(dashboard, sourceLabel) {
  ...
  const schemaVersion = readRequiredField(dashboard, "schemaVersion", `${sourceLabel}.schemaVersion`);
  ...
  const adoption = readRequiredField(dashboard, "adoption", `${sourceLabel}.adoption`);
  if (!isRecord(adoption)) {
    throw new Error(`Missing required object field: ${sourceLabel}.adoption.`);
  }
  const quality = readRequiredField(dashboard, "quality", `${sourceLabel}.quality`);
  if (!isRecord(quality)) {
    throw new Error(`Missing required object field: ${sourceLabel}.quality.`);
  }
  const incidentRate = readRequiredField(dashboard, "incidentRate", `${sourceLabel}.incidentRate`);
  if (!isRecord(incidentRate)) {
    throw new Error(`Missing required object field: ${sourceLabel}.incidentRate.`);
  }
  const recallQuality = readRequiredField(dashboard, "recallQuality", `${sourceLabel}.recallQuality`);
  if (!isRecord(recallQuality)) {
    throw new Error(`Missing required object field: ${sourceLabel}.recallQuality.`);
  }
  const usefulness = readRequiredField(dashboard, "usefulness", `${sourceLabel}.usefulness`);
  if (!isRecord(usefulness)) {
    throw new Error(`Missing required object field: ${sourceLabel}.usefulness.`);
  }

  return {
    requestVolume: readNonNegativeInteger(
      readRequiredField(adoption, "requestVolume", `${sourceLabel}.adoption.requestVolume`),
      `${sourceLabel}.adoption.requestVolume`,
    ),
    failureRate: readRate(
      readRequiredField(adoption, "failureRate", `${sourceLabel}.adoption.failureRate`),
      `${sourceLabel}.adoption.failureRate`,
    ),
    p95LatencyMs: readLatency(
      readRequiredField(quality, "p95LatencyMs", `${sourceLabel}.quality.p95LatencyMs`),
      `${sourceLabel}.quality.p95LatencyMs`,
    ),
    p95LatencyEstimateMs: readLatency(
      readRequiredField(quality, "p95LatencyEstimateMs", `${sourceLabel}.quality.p95LatencyEstimateMs`),
      `${sourceLabel}.quality.p95LatencyEstimateMs`,
    ),
    incidentsPer1kRequests: readNonNegativeNumber(
      readRequiredField(incidentRate, "incidentsPer1kRequests", `${sourceLabel}.incidentRate.incidentsPer1kRequests`),
      `${sourceLabel}.incidentRate.incidentsPer1kRequests`,
    ),
    incidentCount: readNonNegativeInteger(
      readRequiredField(incidentRate, "incidentCount", `${sourceLabel}.incidentRate.incidentCount`),
      `${sourceLabel}.incidentRate.incidentCount`,
    ),
    rollbackIncidentCount: readNonNegativeInteger(
      readRequiredField(incidentRate, "rollbackIncidentCount", `${sourceLabel}.incidentRate.rollbackIncidentCount`),
      `${sourceLabel}.incidentRate.rollbackIncidentCount`,
    ),
    recallFailureRate: readRate(
      readRequiredField(recallQuality, "recallFailureRate", `${sourceLabel}.recallQuality.recallFailureRate`),
      `${sourceLabel}.recallQuality.recallFailureRate`,
    ),
    qualityFeedbackCount: readNonNegativeInteger(
      readRequiredField(recallQuality, "qualityFeedbackCount", `${sourceLabel}.recallQuality.qualityFeedbackCount`),
      `${sourceLabel}.recallQuality.qualityFeedbackCount`,
    ),
    feedbackCount: readNonNegativeInteger(
      readRequiredField(usefulness, "feedbackCount", `${sourceLabel}.usefulness.feedbackCount`),
      `${sourceLabel}.usefulness.feedbackCount`,
    ),
    highSeverityCount: readNonNegativeInteger(
      readRequiredField(usefulness, "highSeverityCount", `${sourceLabel}.usefulness.highSeverityCount`),
...
function aggregateSummaries(summaries) {
  ...
  for (const summary of summaries) {
    ...
    if (summary.p95LatencyMs !== null && summary.latencySampleSize > 0) {
      weightedP95LatencyNumerator += summary.p95LatencyMs * summary.latencySampleSize;
      weightedP95LatencyDenominator += summary.latencySampleSize;
      p95LatencyMs = p95LatencyMs === null ? summary.p95LatencyMs : Math.max(p95LatencyMs, summary.p95LatencyMs);
    }

    for (const count of summary.anomalyHistogram.values()) {
      anomalyCount += count;
    }
    for (const [policyDecision, count] of summary.policyDecisionHistogram.entries()) {
      const normalizedDecision = normalizeToken(policyDecision);
      if (normalizedDecision === "review" || normalizedDecision === "deny" || normalizedDecision === "denied") {
  ...
  return {
    ...
    p95LatencyEstimateMs:
      weightedP95LatencyDenominator > 0 ? round(weightedP95LatencyNumerator / weightedP95LatencyDenominator, 3) : null,
    recallRequestVolume,
    recallFailureCount,
...
function aggregateDashboards(dashboards) {
  ...
  for (const dashboard of dashboards) {
    ...
    if (dashboard.p95LatencyMs !== null) {
      p95LatencyMs = p95LatencyMs === null ? dashboard.p95LatencyMs : Math.max(p95LatencyMs, dashboard.p95LatencyMs);
    }
    if (dashboard.p95LatencyEstimateMs !== null) {
      p95LatencyMs =
        p95LatencyMs === null ? dashboard.p95LatencyEstimateMs : Math.max(p95LatencyMs, dashboard.p95LatencyEstimateMs);
    }
    recallFailureRate = Math.max(recallFailureRate, dashboard.recallFailureRate);
    incidentsPer1kRequests = Math.max(incidentsPer1kRequests, dashboard.incidentsPer1kRequests);
    incidentCount = Math.max(incidentCount, dashboard.incidentCount);
    rollbackIncidentCount = Math.max(rollbackIncidentCount, dashboard.rollbackIncidentCount);
    feedbackCount = Math.max(feedbackCount, dashboard.feedbackCount);
    highSeverityCount = Math.max(highSeverityCount, dashboard.highSeverityCount);
    qualityFeedbackCount = Math.max(qualityFeedbackCount, dashboard.qualityFeedbackCount);
  }
...
function normalizeFeedbackRecord(record, index) {
  ...
  const category = normalizeToken(readRequiredField(record, "category", `feedbackRecords[${index}].category`));
  const severity = normalizeToken(readRequiredField(record, "severity", `feedbackRecords[${index}].severity`));
  const action = normalizeToken(record.action ?? "none");
  if (!category) {
    throw new Error(`feedbackRecords[${index}] is missing required field: category.`);
  }
  if (!severity) {
    throw new Error(`feedbackRecords[${index}] is missing required field: severity.`);
  }
...
function aggregateFeedback(feedbackRecords, requestVolume) {
  ...
  for (let index = 0; index < feedbackRecords.length; index += 1) {
    ...
    if (feedback.category === "quality" || feedback.category === "ux" || feedback.category === "recall") {
...
function mergeObservedMetrics(summarySignals, dashboardSignals, feedbackSignals) {
  const requestVolume = Math.max(
    summarySignals.requestVolume,
    dashboardSignals?.requestVolume ?? 0,
  );
  const failureRate = Math.max(summarySignals.failureRate, dashboardSignals?.failureRate ?? 0);
  ...
  const incidentCount = Math.max(feedbackSignals.incidentCount, dashboardSignals?.incidentCount ?? 0);
  const rollbackIncidentCount = Math.max(
    feedbackSignals.rollbackIncidentCount,
    dashboardSignals?.rollbackIncidentCount ?? 0,
  );
  const feedbackCount = Math.max(feedbackSignals.feedbackCount, dashboardSignals?.feedbackCount ?? 0);
...
function computePressures(observedMetrics) {
  const failurePressure = clamp((observedMetrics.failureRate - 0.005) / 0.025, 0, 1);
  const latencyPressure = clamp(((observedMetrics.p95LatencyMs ?? 0) - 250) / 350, 0, 1);
  const recallPressure = clamp((observedMetrics.recallFailureRate - 0.02) / 0.18, 0, 1);
  const anomalyPressure = clamp(observedMetrics.anomalyPer1kRequests / 15, 0, 1);
  const incidentPressure = clamp(observedMetrics.incidentsPer1kRequests / 8, 0, 1);
  const feedbackPressure = clamp(
    (observedMetrics.highSeverityFeedbackRate + observedMetrics.rollbackSignalRate) / 0.2,
    0,
    1,
  );
...
function boundedNormalizeWeights(rawWeights) {
  const weights = new Map(RANKING_WEIGHT_ORDER.map((key) => [key, rawWeights[key] ?? 0]));
  const fixed = new Map();
  const epsilon = 1e-12;
  ...
  while (true) {
    ...
    const freeRawSum = freeKeys.reduce((sum, key) => sum + Math.max(0, weights.get(key)), 0);
  ...
  const sum = RANKING_WEIGHT_ORDER.reduce((total, key) => total + weights.get(key), 0);
  if (sum <= 0) {
...
function recommendRankingWeights(pressures) {
  const recencyDelta = 0.1 * pressures.latencyPressure + 0.06 * pressures.anomalyPressure;
  const reliabilityDelta =
    0.12 * pressures.recallPressure +
    0.08 * pressures.failurePressure +
    0.05 * pressures.incidentPressure;
  const policySafetyDelta = 0.08 * pressures.incidentPressure + 0.05 * pressures.feedbackPressure;
  const semanticSimilarityDelta = -(recencyDelta + reliabilityDelta + policySafetyDelta);

  const rawWeights = {
    recency: BASELINE_RANKING_WEIGHTS.recency + recencyDelta,
    semanticSimilarity: BASELINE_RANKING_WEIGHTS.semanticSimilarity + semanticSimilarityDelta,
...
function recommendDecayPolicy(pressures) {
  ...
  const minRetentionDays = clamp(
    Math.round(
      BASELINE_DECAY_POLICY.minRetentionDays + 2 * pressures.recallPressure - 2 * pressures.incidentPressure,
    ),
    1,
    14,
  );
  ...
  const recencyBoostWindowDays = clamp(
    Math.round(
      BASELINE_DECAY_POLICY.recencyBoostWindowDays + 3 * pressures.latencyPressure + 2 * pressures.recallPressure,
    ),
    3,
    14,
  );

  const boundedStaleAfter = clamp(staleAfterDays, minRetentionDays, maxRetentionDays);
...
function buildGuardrails(observedMetrics, pressures) {
  return {
    ...
    maxFailureRate: round(clamp(0.0075 + 0.004 * Math.max(pressures.failurePressure, pressures.recallPressure), 0.005, 0.015)),
    maxRecallFailureRate: round(clamp(0.04 + 0.04 * pressures.recallPressure, 0.03, 0.12)),
    maxP95LatencyMs: Math.round(clamp(275 + 100 * pressures.latencyPressure + 75 * pressures.incidentPressure, 250, 450)),
    maxIncidentsPer1kRequests: round(clamp(1.5 + 1.5 * pressures.incidentPressure, 1, 4)),
    maxAnomaliesPer1kRequests: round(clamp(4 + 4 * pressures.anomalyPressure, 3, 10)),
    maxPolicyReviewRate: round(clamp(0.06 + 0.08 * pressures.incidentPressure, 0.05, 0.2)),
    abortOnRollbackSignal: true,
    rollbackSignalDetected: observedMetrics.rollbackIncidentCount > 0,
  };
}

function buildRationale(observedMetrics, pressures, rankingWeights, decayPolicy) {
  const reasonCodes = [];
  ...
  pushReason("RECALL_FAILURE_ELEVATED", observedMetrics.recallFailureRate > 0.05, {
    ...
    threshold: 0.05,
    impact: "Increase reliability weight and adjust retention window for recall stability.",
  });
  ...
  pushReason("LATENCY_ELEVATED", (observedMetrics.p95LatencyMs ?? 0) > 300, {
    ...
    threshold: 300,
    impact: "Increase recency contribution and shorten stale horizon under load.",
  });
  pushReason("ANOMALY_RATE_ELEVATED", observedMetrics.anomalyPer1kRequests > 4, {
  ...
  pushReason("INCIDENT_RATE_ELEVATED", observedMetrics.incidentsPer1kRequests > 2, {
    ...
    threshold: 2,
    impact: "Increase policy safety weight and reduce retention aggressiveness ceiling.",
  });
  pushReason("HIGH_SEVERITY_FEEDBACK_ELEVATED", observedMetrics.highSeverityFeedbackRate > 0.2, {
    severity: observedMetrics.highSeverityFeedbackRate > 0.35 ? "high" : "medium",
    metric: "highSeverityFeedbackRate",
    observed: observedMetrics.highSeverityFeedbackRate,
    threshold: 0.2,
    impact: "Bias ranking toward safer matches and stricter rollout guardrails.",
  });
  pushReason("ROLLBACK_SIGNAL_PRESENT", observedMetrics.rollbackIncidentCount > 0, {
    severity: "high",
  ...
  if (reasonCodes.length === 0) {
    reasonCodes.push({
      ...
      observed: 0,
      threshold: 0,
      impact: "Keep recommendations near baseline while maintaining standard guardrails.",
    });
  }
  ...
  return {
    ...
    rankingDelta: {
      ...
      semanticSimilarity: round(rankingWeights.semanticSimilarity - BASELINE_RANKING_WEIGHTS.semanticSimilarity),
      reliability: round(rankingWeights.reliability - BASELINE_RANKING_WEIGHTS.reliability),
      policySafety: round(rankingWeights.policySafety - BASELINE_RANKING_WEIGHTS.policySafety),
    },
    decayDelta: {
      halfLifeDays: decayPolicy.halfLifeDays - BASELINE_DECAY_POLICY.halfLifeDays,
      minRetentionDays: decayPolicy.minRetentionDays - BASELINE_DECAY_POLICY.minRetentionDays,
      staleAfterDays: decayPolicy.staleAfterDays - BASELINE_DECAY_POLICY.staleAfterDays,
      maxRetentionDays: decayPolicy.maxRetentionDays - BASELINE_DECAY_POLICY.maxRetentionDays,
      recencyBoostWindowDays:
        decayPolicy.recencyBoostWindowDays - BASELINE_DECAY_POLICY.recencyBoostWindowDays,
    },
  };
}
...
export function generateRankingDecayTuning(
  ...
  const feedbackSignals = aggregateFeedback(feedbackRecords, summarySignals.requestVolume);

  const observedMetrics = mergeObservedMetrics(summarySignals, dashboardSignals, feedbackSignals);
  ...
  return {
    ...
    guardrails,
    rationale: buildRationale(observedMetrics, pressures, recommendedRankingWeights, recommendedDecayPolicy),
  };
}
...
function parseArgs(argv) {
  ...
  if (!parsed.help && parsed.input.length === 0) {
    throw new Error("Missing required --input <path> argument (repeat for multiple summary files).");
  }
  return parsed;
}
...
function printUsage() {
  process.stdout.write(
    [
      "Usage:",
      "  node scripts/generate-ranking-decay-tuning.mjs --input <summary.json> [--input <summary.json> ...]",
      ...
      "",
      "Example:",
      "  node scripts/generate-ranking-decay-tuning.mjs --input docs/reports/pilot-rollout/final-summary.json \\",
...
function parseJsonObject(content, sourceLabel, artifactLabel) {
  ...
  try {
    ...
    throw new Error(`Invalid ${artifactLabel} JSON (${sourceLabel}): ${message}`);
  }
  if (!isRecord(parsed)) {
    throw new Error(`${artifactLabel} input must be a JSON object (${sourceLabel}).`);
  }
  return parsed;
}
...
export async function main(argv = process.argv.slice(2)) {
  ...
  const recommendation = generateRankingDecayTuning(summaries, { dashboards, feedbackRecords });
