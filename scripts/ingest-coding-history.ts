...
} from "../apps/api/src/persistence.ts";
...
const RULE_DIRECTIVE_PATTERN =
  /\b(always|must|should|need to|make sure|prefer|ensure|verify|run|avoid)\b/i;
...
const ANTIPATTERN_FAILURE_PATTERN =
  /\b(fail(?:ed|ure)?|blocked|timeout|flaky|regression|broken|error|crash)\b/i;
...
function looksLikeNoiseBlob(value) {
  ...
  if (
    JSONISH_NOISE_PATTERN.test(text) &&
    (text.match(/[:{}[\]"]/g)?.length ?? 0) > 24
  ) {
    return true;
  }
...
function isActionableAntiPatternLine(line) {
  ...
  if (
    !ANTIPATTERN_FAILURE_PATTERN.test(text) ||
    !ANTIPATTERN_WORKFLOW_PATTERN.test(text)
  ) {
    return false;
  }
  return true;
...
function mergeRuleCandidates(
  ...
  return [...merged.values()]
    .sort(
      (left, right) =>
        right.frequency - left.frequency || right.confidence - left.confidence
    )
...
function buildHeuristicAntiPatternNotes(
  ...
  const notes = [...buckets.values()]
    ...
    .sort(
      (left, right) =>
        right.count - left.count || left.note.localeCompare(right.note)
    )
    .slice(0, maxNotes * 2)
...
async function main(argv = process.argv.slice(2)) {
  ...
  const planEvents = loadCassNcmPlanEvents({
    storeId: parsed.storeId,
    profile: parsed.profile,
  });
  ...
  const learning = await executeOperationWithSharedState({
    ...
    executor: () => {
      ...
      const heuristicCandidates = buildHeuristicRuleCandidates(
        allEvents,
        MAX_RULE_CANDIDATES
      );
      ...
      const combinedCandidates = mergeRuleCandidates(
        [canonicalCandidates],
        MAX_RULE_CANDIDATES
      );
      ...
      const curatedCandidates = combinedCandidates.filter(
        (_, index) => validate.validations[index]?.valid
      );
  ...
  const report = {
    ...
    learning: {
      ...
      highSignalHeuristicCandidatesGenerated:
        learning.highSignalHeuristics.length,
      auditChecks: learning.audit.checks,
      topRules: learning.exported.playbook.topRules,
      antiPatterns: extractActionableAntiPatterns(
        learning.exported.playbook.antiPatterns,
        MAX_ANTIPATTERN_NOTES
      ),
...
main().then(
  ...
  (error) => {
    process.stderr.write(
      `ingest-coding-history failed: ${error instanceof Error ? error.message : String(error)}\n`
    );
    process.exitCode = 1;
  }
);
