...
function readTelemetryWindow(value, label, requestVolume) {
  ...
  const start = readTimestampOrNull(readRequiredField(value, "start", `${label}.start`), `${label}.start`);
  const end = readTimestampOrNull(readRequiredField(value, "end", `${label}.end`), `${label}.end`);

  if ((start === null) !== (end === null)) {
    throw new Error(`Telemetry window must contain both start and end timestamps or both null (${label}).`);
  }
  ...
  if (requestVolume > 0 && (!start || !end)) {
    throw new Error(`Telemetry window cannot be null when requestVolume > 0 (${label}).`);
  }
  if (requestVolume === 0 && (start || end)) {
    throw new Error(`Telemetry window must be null when requestVolume is 0 (${label}).`);
  }
  return { start, end };
}
...
function readPerOperation(value, label) {
  ...
  for (const [operation, stats] of Object.entries(value)) {
    ...
    readRate(readRequiredField(stats, "successRate", `${prefix}.successRate`), `${prefix}.successRate`);
    readRate(readRequiredField(stats, "failureRate", `${prefix}.failureRate`), `${prefix}.failureRate`);
    ...
    const latencySampleSize = readNonNegativeInteger(
      readRequiredField(stats, "latencySampleSize", `${prefix}.latencySampleSize`),
      `${prefix}.latencySampleSize`,
    );
    const failureCodeHistogram = readHistogram(
      readRequiredField(stats, "failureCodeHistogram", `${prefix}.failureCodeHistogram`),
...
function readPilotSummary(summary, sourceLabel) {
  ...
  const invalidEventCount = readNonNegativeInteger(
    readRequiredField(summary, "invalidEventCount", `${sourceLabel}.invalidEventCount`),
    `${sourceLabel}.invalidEventCount`,
  );

  const operationHistogram = readHistogram(
    readRequiredField(summary, "operationHistogram", `${sourceLabel}.operationHistogram`),
    `${sourceLabel}.operationHistogram`,
  );
  const failureCodeHistogram = readHistogram(
    readRequiredField(summary, "failureCodeHistogram", `${sourceLabel}.failureCodeHistogram`),
  ...
  const projectHistogram = readHistogram(
    readRequiredField(summary, "projectHistogram", `${sourceLabel}.projectHistogram`),
  ...
  const telemetryWindow = readTelemetryWindow(
    readRequiredField(summary, "telemetryWindow", `${sourceLabel}.telemetryWindow`),
...
function parseFeedbackObject(record, index, sourceLabel) {
  if (!isRecord(record)) {
    throw new Error(`${sourceLabel} feedback entry ${index} must be a JSON object.`);
  }

  const timestamp = toIsoTimestamp(record.timestamp);
  if (!timestamp) {
    throw new Error(`${sourceLabel} feedback entry ${index} is missing required RFC3339 timestamp.`);
  }

  const category = normalizeSliceLabel(record.category, "");
  if (!category) {
    throw new Error(`${sourceLabel} feedback entry ${index} is missing required field: category.`);
  }

  const severity = normalizeSliceLabel(record.severity, "");
  if (!severity) {
    throw new Error(`${sourceLabel} feedback entry ${index} is missing required field: severity.`);
  }
...
function parseFeedbackLine(line, lineNumber, sourceLabel) {
  ...
  try {
    ...
    throw new Error(`Invalid JSON feedback entry on line ${lineNumber} (${sourceLabel}): ${message}`);
  }
  return parseFeedbackObject(parsed, lineNumber - 1, sourceLabel);
}

export function parseFeedbackEvents(source, { allowInvalid = false, sourceLabel = "feedback" } = {}) {
  ...
  if (trimmed.startsWith("[")) {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : String(error);
      throw new Error(`Invalid JSON feedback array (${sourceLabel}): ${message}`);
    }
    if (!Array.isArray(parsed)) {
      throw new Error(`Feedback input must be an array or NDJSON stream of objects (${sourceLabel}).`);
    }
...
function histogramToSortedObject(histogram) {
  const entries = [...histogram.entries()].sort((left, right) => compareStrings(left[0], right[0]));
...
export function generatePilotKpiDashboard(summaries, feedbackRecords = [], { allowInvalid = false } = {}) {
  ...
  for (let index = 0; index < summaries.length; index += 1) {
    ...
    if (summary.p95LatencyMs !== null && summary.latencySampleSize > 0) {
      ...
      maxP95LatencyMs =
        maxP95LatencyMs === null ? summary.p95LatencyMs : Math.max(maxP95LatencyMs, summary.p95LatencyMs);
    }
    ...
    mergeHistogram(recallAnomalyHistogram, summary.anomalyHistogram, (key) => isRecallKey(key));
  ...
  let invalidFeedbackCount =
    Number.isInteger(feedbackRecords.invalidCount) && feedbackRecords.invalidCount > 0
  ...
  for (let index = 0; index < feedbackRecords.length; index += 1) {
    let parsedFeedback;
    try {
      parsedFeedback = parseFeedbackObject(feedbackRecords[index], index, "feedback");
    } catch (error) {
      if (!allowInvalid) {
        throw error;
      }
    ...
    if (parsedFeedback.category === "quality" || parsedFeedback.category === "ux") {
  ...
  const weightedP95LatencyEstimateMs =
    p95WeightedDenominator > 0 ? Number((p95WeightedNumerator / p95WeightedDenominator).toFixed(3)) : null;
  ...
  return {
    ...
    recallQuality: {
      ...
      recallFailureCodeHistogram: histogramToSortedObject(recallFailureCodeHistogram),
...
function printUsage() {
  process.stdout.write(
    [
      "Usage:",
      "  node scripts/generate-pilot-kpi-dashboard.mjs --input <summary.json> [--input <summary.json> ...]",
      ...
      "",
      "Example:",
      "  node scripts/generate-pilot-kpi-dashboard.mjs --input day1-summary.json --input day2-summary.json \\",
...
export async function main(argv = process.argv.slice(2)) {
  ...
  for (const feedbackPath of parsedArgs.feedback) {
    ...
    feedbackInvalidCount +=
      Number.isInteger(parsedFeedback.invalidCount) && parsedFeedback.invalidCount > 0
