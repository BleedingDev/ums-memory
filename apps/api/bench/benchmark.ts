...
function runSchedulingVolumeBenchmark(volume: any) {
  ...
  const rebalanceActiveLimit = Math.min(
    128,
    Math.max(16, Math.floor(volume / 2))
  );
...
const updateNearConstant = computeNearConstantGate(
  schedulingVolumeResults,
  "updateLatency"
);
const clockNearConstant = computeNearConstantGate(
  schedulingVolumeResults,
  "clockLatency"
);
const rebalanceNearConstant = computeNearConstantGate(
  schedulingVolumeResults,
  "rebalanceLatency"
);
...
const versionedJsonPath = resolve(
  outputDir,
  `phase3-review-scheduling-latency-gate.${version}.json`
);
const latestJsonPath = resolve(
  outputDir,
  "phase3-review-scheduling-latency-gate.latest.json"
);
const versionedMdPath = resolve(
  outputDir,
  `phase3-review-scheduling-latency-gate.${version}.md`
);
const latestMdPath = resolve(
  outputDir,
  "phase3-review-scheduling-latency-gate.latest.md"
);
