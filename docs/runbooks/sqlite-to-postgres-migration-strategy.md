...
- `migrations.ts` (versioned migration list, same semantic versions as SQLite v1-v4+)
...
| 2. Postgres Schema + Migrations   | Portability mapping approved                                         | Postgres migrations reach semantic parity with SQLite v1-v4; drift checks/tests pass; append-only `audit_events` and idempotency-ledger invariants validated in parity tests | Migration checksums + deterministic ordering tests; fail-fast on drift                              |
...
3. `ums-memory-dd2.5`: define enterprise identity lifecycle and role sync contracts in [Enterprise SSO/SCIM Integration Plan](./enterprise-sso-scim-integration-plan.md).
4. Add Postgres schema + migration parity test suite mirroring SQLite migration tests.
5. Add dual-run telemetry fields (request hash, backend result digest, mismatch category) to storage operation logs.
