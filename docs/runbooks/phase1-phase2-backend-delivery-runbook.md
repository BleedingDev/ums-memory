...
2. Operational benchmark checks:
   Latency SLO validation uses benchmark reports from `bun run bench:ums` and is reviewed as part of release hardening.
...
```bash
UMS_IMPL_MODULE=./apps/api/src/ums/engine.ts UMS_IMPL_EXPORT=createUmsEngine bun test tests/unit/*.test.ts tests/integration/*.test.ts
UMS_IMPL_MODULE=./apps/api/src/ums/engine.ts UMS_IMPL_EXPORT=createUmsEngine bun benchmarks/ums-latency-benchmark.ts
```
