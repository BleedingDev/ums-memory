...
4. Managed login posture:
   Managed/company accounts use browser/device/SSO style login flows where possible. The daemon stores refresh/session material in secure storage and keeps short-lived access tokens in memory, not in the config file. CLI bootstrap may generate a platform-native default `credentialRef`, and `ums account login` writes only to secure storage.
...
13. Daemon-first lifecycle vocabulary is frozen:
    The external CLI surface is daemon-first and CLI-first across both solo and managed modes. The canonical lifecycle command families are:
    - `ums install|uninstall`
    - `ums start|stop|restart|status|logs`
    - `ums sync|sync-daemon`
    - `ums config init|validate|doctor`
    - `ums account ...`, `ums memory ...`, `ums route ...`
    - `ums agent skill|bootstrap`
14. Remote MCP is explicitly deferred:
    Remote MCP is not part of the launch-critical control plane. Any future hosted MCP surface must remain additive and cannot replace the CLI-first daemon contract.
...
8. Lifecycle contract:
   The daemon-first command families and same-external-contract rule for solo versus managed users are frozen.
9. Remote MCP deferral:
   Hosted or remote MCP remains out of scope until the daemon-first CLI runtime is stable.
...
- Forces daemon lifecycle work to preserve one CLI contract across local and managed runtimes instead of drifting into tool-specific control planes.
